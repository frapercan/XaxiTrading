...
URL2 = "https://api.bitfinex.com/v2"
...
def ticker(symbol='iotbtc'): # gets the innermost bid and asks and information on the most recent trade.
...
def tickers(symbols=['tBTCUSD','tIOTUSD']): # gets the innermost bid and asks and information on the most recent trade.

	r = requests.get(URL2 + "/tickers?symbols=" + ",".join(symbols ),verify=True) # <== UPDATED TO LATEST VERSION OF BFX!
	rep = r.json()
#	print(URL2 + "/tickers?symbols=" + ",".join(symbols ))
...
def stats(symbol='iotbtc'): # Various statistics about the requested pairs.
...
def today(symbol='iotbtc'): # today's low, high and volume.
...
def payloadPacker(payload): # packs and signs the payload of the request.
	...
	bj = j.encode()
	data = base64.standard_b64encode(bj)
...
def place_order(amount, price, side, ord_type, symbol='iotbtc', exchange='bitfinex'): # submit a new order.
